...
import { RoleBadge, Spinner, EmptyState, PageHeader, formatDate } from '../components/ui'
...
export default function Users() {
  ...
  const toggleActive = async (u: User) => {
    if (u.id === currentUser?.id) return alert("You can't deactivate yourself.")
      ...
      <div className="card">
        {loading ? (
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {users.map((u) => (
                <tr key={u.id} className="border-b border-dark-600 last:border-0 hover:bg-dark-600/40 transition-colors">
                  ...
                  <td className="py-3 pr-4">
                    <span className={`text-xs px-2 py-0.5 rounded font-medium ${u.isActive ? 'bg-emerald-950 text-emerald-400' : 'bg-slate-800 text-slate-500'}`}>
                      {u.isActive ? 'Active' : 'Inactive'}
                    </span>
                  </td>
                  <td className="py-3 pr-4 text-slate-500 text-xs whitespace-nowrap">{formatDate(u.createdAt)}</td>
